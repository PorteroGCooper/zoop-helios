...
//////////////////////////////////////////////
	...
	if(!isset($_SERVER["PATH_INFO"])) {
		...
		// This ensures Zoop works as a fastcgi script 
	 ...
	 * this handles spaces in the path_info. I think we shouldn't have to support it, but a bug made
	 * it possible. we'll support it until we know that no one uses that bug.
	 */
	...
	if(Config::get('zoop.app.security.hide_post', true)) {
		unset($_POST);
	}

	// For security reasons, hide direct access to the $_GET variable.
	$GLOBALS['_GET_UNFILTERED'] = $_GET;
