...
class ComplexInsert {
	...
	function go($inExecute = 1) {
		if(count($this->fields) == 0 || count($this->rows) == 0) return 0;
		...
		while(list($rowKey, $thisRow) = each($this->rows)) {
			...
			foreach($thisRow as $fieldKey => $thisField) {
				...
				if($thisField === null) {
					...
					} elseif((gettype($fieldDesc) == 'array') && ($fieldDesc[0] == 'date')) {
