...
function validateForm( f, bConfirm, bDisable, bDisableR, groupError, errorMode )
{
	...
	errorMode	= ( typeof errorMode != 'undefined' ) ? parseInt( errorMode, 10 ) : 16;
	...
	while ( elem = f.elements[i++] )
	{
		...
		/*
		if(elem.type == 'TEXTAREA' || elem.type == 'textarea')
		{
			//alert('skipping a textarea');
			continue;
		}
		*/
		...
		if ( !( typeof fvCode == 'undefined' || fvCode == null || fvCode == "" ) )
		{
			...
			//	Check for modifiers
			//alert(lastElemArray(params));
			switch( lastElemArray(params))
			{
				case 'bok'	:	//	bok requested
					params = arrayReduce(params,1, 1 );
					elem.bok = true;
					break;
			...
			var func = new Function( "obj", "method", "obj[method]( " + arrayToArgString(params) + " );" );
			//alert(func);
			//alert(type);
			func( f.fv, type );
			//alert(elem.validated);
			//alert(groupError);
...
fValidate.prototype.throwError = function( args, which )
{
	...
	if ( this.groupError )
	{
		...
		arrayPush(this.errors, {'elem':elem, 'msg': error} );
	}
	else
	{
...
fValidate.prototype.showError = function( emsg, last, elem )
{
	...
	function alertError()
	{
		if ( self.groupError ) ArrayPush(self.showErrors, emsg );
		else alert( emsg );
...
fValidate.prototype.typeMismatch = function()
{
	...
	{
		//alert('argument: ' + arguments[i]);
		//alert('type: ' + this.elem.type);
		type	= pats[arguments[i]];
		regex	= new RegExp( type );
		result	+= ( regex.test( this.elem.type ) ) ? "1" : "0";
		key		+= "0";
		//alert(expected.length);
		arrayPush(expected, type );
		//alert(expected.length);
	}
	//alert(key);
	//alert(result);
		...
		case 'checkbox' :
			if ( typeof elem.length == 'undefined' )
			{
				...
				for ( var i = 0; i < elem.length; i++ )
				{
					...
					if ( elem[i][choice] )
					{
						arrayPush(returnValues, elem[i].value );
					}
				}
				return returnValues;
			}
...
function arrayReduce(obj, b, e)
{
	var a = new Array();
	var count = 0;
	for ( var i = b; i < obj.length - e; i++ )
	{
		a[count++] = obj[i];
	}
	return a;
...
function arrayToArgString(obj)
{
	var a = new Array();
	for ( var i = 0; i < obj.length; i++ )
	{
		//alert('in to arg string ' + obj[i]);
		//alert('argString ' + a.toString());
		arrayPush(a, "'" + obj[i] + "'" );
	}
	//alert(a.toString());
	return a.toString();
...
function arrayPush(obj, arguments)
{
	var arg, i = 0;
...
{
//	alert (this.guiControlsToUserReadable());
	return this.guiControlsToUserReadable().replace( /\_/g, " ").replace( /\[|\]/g, "" );
}

/*	Removes the guiControls Naming from a label for human-reading
------------------------------------------- */
String.prototype.guiControlsToUserReadable = function()
{
	var myregexp = /controls\[.*\]\[(.*)\]\[text\]/;
